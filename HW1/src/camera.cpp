...
void Camera::updateViewMatrix() {
  ...
  /*** 1-1.1 ***/
  /*** Rotate the original front and rotation by multiplying this->rotation  ***/
  glm::vec3 newFront = original_front * rotation;
  glm::vec3 newUp = original_up * rotation;

  /*** 1-1.2 ***/
  /*** right is calculated by cross the new value of front and up vector ***/
  right = glm::normalize(glm::cross(newFront, newUp));

  /*** 1-1.3 ***/
  /*** First of all, get the center of the object ***/
  /*** Secondly, use the glm::lookAt to get the view matrix ***/
  glm::vec3 target = position + newFront;
  viewMatrix = glm::lookAt(position, target, newUp);
}
...
void Camera::updateProjectionMatrix(float aspectRatio) {
  ...
  /*** 1-2 ***/
  /*** Projection matrix remain unchange even if the camera is moving ***/
